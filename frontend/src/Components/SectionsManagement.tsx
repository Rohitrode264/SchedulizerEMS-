...
import { Building2, GraduationCap, Users, Plus, X, Pencil, Trash2 } from 'lucide-react';
...
  };
  ...
  const handleEditSection = (sectionId: string) => {
    ...
    // Find the section to edit and pre-fill form fields
    const sectionToEdit = fetchedSections.find(s => s.id === sectionId);
    if (sectionToEdit) {
      setDepartmentName(sectionToEdit.departmentCode || '');
      if (sectionToEdit.batchYearRange) {
        const [start, end] = sectionToEdit.batchYearRange.split('-');
        setBatchYearStart(start || '');
        setBatchYearEnd(end || '');
      }
      setNumberOfSections(1); // Set to 1 since we're editing a single section
      // Load the section to be edited into the local state
      setSections([
        {
          id: sectionToEdit.id,
          name: sectionToEdit.name,
          batches: sectionToEdit.batches,
...
                      </div>
                      <button
                        ...
                        className="text-xs p-2 rounded-md bg-blue-600 text-white hover:bg-blue-700"
                      >
                        <Pencil className="w-4 h-4" />
                      </button>
                      <button
                        onClick={() => removeSection(section.id)}
                        className="text-xs p-2 rounded-md bg-red-600 text-white hover:bg-red-700"
                      >
...
                        </div>
                        <button
                          ...
                          className="text-xs p-1 rounded-md bg-red-100 text-red-700 hover:bg-red-200"
                        >
...
                <GraduationCap className="w-5 h-5" />
                {editingSectionId ? 'Edit Section Configuration' : fetchedSections.length > 0 ? 'Add New Section Configuration' : 'Department Configuration'}
              </h3>
              {(fetchedSections.length > 0 || editingSectionId) && (
                <button
...
                isNewSection={!editingSectionId && section.id.startsWith('temp-')}
...
              className="ml-2 p-2 rounded-md bg-red-600 text-white hover:bg-red-700"
            >
              <Trash2 className="w-4 h-4" />
            </button>
          )}
